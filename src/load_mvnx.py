...
Published by Movilla with minor changes by 
Andy Pohl as part of the Movilla Biomechanics Challenge 2023
"""
...
def load_mvnx(file_name):
    ...
    mvnx_file.file_data["meta_data"]["color"] = subject_element.get(
        "torsoColor")
    mvnx_file.file_data["meta_data"]["sample_rate"] = subject_element.get(
        "frameRate")
    mvnx_file.file_data["meta_data"]["rec_date"] = subject_element.get(
        "recDate")
    ...
    mvnx_file.file_data["meta_data"]["scenario"] = subject_element.get(
        "userScenario")
    ...
    # for later convenience on retrieving segment names
    mvnx_file.create_index_to_segment_dict()
    ...
    if ergo_joints_element is not None:
        mvnx_file.file_data["ergo_joints"] = parse_ergo_joints(
            ergo_joints_element)
    ...
    for side in mvnx_file.file_data["finger_joints"]["elements"]:
        ...
        if finger_joints_element is not None:
            ...
            finger_joints = parse_joints(
                finger_joints_element, finger_segments)
    ...
    # Reset window so frame count displays total frame count
    mvnx_file.reset_frame_window()
    return mvnx_file
...
def parse_ergo_joints(ergo_joints_element):
    ...
    """
    ergo_joint_elements = ergo_joints_element.findall(
        "mvn:ergonomicJointAngle", ns)
...
def parse_segments(segment_elements):
    ...
    for segment_index in range(len(segment_elements)):
        ...
        for point_index in range(len(point_elements)):
            ...
            if (
                np.sqrt(
                    np.sum([sq * sq for sq in segment["points_mvn"][point_name]]))
                < 0.00001
            ):
...
def parse_frames(frames_element, mvnx_file):
    ...
    def get_count(element): return int(
    ...
    frames["finger_joint_count"] = get_count(
        frames_element.get("fingerJointCount"))
...
def get_segment_data_from_frame(frame_element, segment_names):
    ...
    angular_velocity = frame_element_as_floats(
        frame_element, "angularVelocity")
    angular_acceleration = frame_element_as_floats(
        frame_element, "angularAcceleration")
    ...
    for index in range(len(segment_names)):
        ...
        segment_data[segment_name]["ang_vel"] = get_3d_vector(
    ...
    if center_of_mass:
        ...
        for com_field in segment_data["com"]:
            segment_data["com"][com_field] = get_3d_vector(
...
def get_sensor_data_from_frame(frame_element, sensor_names):
    ...
    magnetic_field = frame_element_as_floats(
        frame_element, "sensorMagneticField")
    ...
    for index in range(len(sensor_names)):
        ...
        sensor_data[sensor_name]["acc"] = get_3d_vector(
            free_accelerations, index)
...
def get_finger_data_from_frame(frame_element, finger_segment_names):
    ...
    for side in finger_data:
        ...
        for index in range(len(finger_segment_names[side])):
            ...
            finger_data[side][finger_name]["ori"] = get_4d_vector(
                orientations, index)
            finger_data[side][finger_name]["pos"] = get_3d_vector(
                offsets, index)
